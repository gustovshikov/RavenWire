#!/usr/bin/env bash
# dev-env/gen-traffic.sh — Generate test traffic on veth1 for spike validation
#
# Run inside the VM: sudo /vagrant/dev-env/gen-traffic.sh
#
# Generates a mix of TCP, UDP, and ICMP traffic on veth1 so that
# Zeek, Suricata, and pcap_ring_writer (all listening on veth0) have
# packets to process during the spike test window.
set -euo pipefail
