...
The current validated state is a single-site pilot MVP candidate, not a broader production/enterprise release. Forwarding telemetry from HealthReport is still placeholder-only. Alert notification delivery, health baselines, live data-flow visualization, canary rollouts, detection-content lifecycle, offline update bundles, production packaging, and multi-manager HA remain roadmap work until explicitly pulled forward.
...
- Health baselines and live data-flow visualization.
...
| 11 | `health-baselines`, `live-data-flow-viz` | Not started; post-MVP. | Builds on historical metrics and alerting for richer observability. |
| 12 | `canary-deploys`, `detection-content-lifecycle`, `offline-update-bundle`, `multi-manager-ha` | Not started; post-MVP. | Adds advanced rollout, air-gap, content lifecycle, and production operations. |
...
The current post-MVP branch implements Historical Metrics on top of Platform Alert Center. Metrics pages use current HealthReport data where available and keep Vector forwarding runtime metrics as explicit placeholders until HealthReport exposes sink telemetry.
