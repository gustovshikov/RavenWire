// pcap_carve_simulator — RavenWire lab helper for pcap_ring_writer carve workflows.
//
// Validates the alert-to-carve workflow end-to-end.
//
...
func main() {
	...
	log.Printf("pcap_carve_simulator: alert in %ds, pre-window=%ds, post-window=%ds",
	...
	preAlertNs := alertTime.Add(-time.Duration(preWindow) * time.Second).UnixNano()

	log.Printf("Simulated alert fired at %s", alertTime.Format(time.RFC3339))

	// Step 2: Send mark_pre_alert to pcap_ring_writer
	log.Printf("Sending mark_pre_alert (timestamp_ms=%d)...", preAlertNs/int64(time.Millisecond))
	resp, err := sendCmd(controlSock, controlCmd{
		Cmd:         "mark_pre_alert",
		TimestampMs: preAlertNs / int64(time.Millisecond),
	})
	if err != nil {
	...
	log.Printf("Sending carve_window: pre=%d post=%d output=%s", preAlertNs, postAlertNs, outputPath)
