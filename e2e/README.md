...
The full profile covers login, dashboard/sensor visibility, pools, forwarding sinks, platform alerts, deployments, rules, BPF, PCAP search/history/detail/manifest/download entry points, support bundles, admin user/role/token pages, and audit filtering/export.
...
Tests create records with an `e2e-` prefix and clean up records they create. Direct SQLite cleanup is disabled unless `E2E_ALLOW_DB_CLEANUP=true`, and it only targets tracked `e2e-` records such as PCAP requests and alert fixtures.
